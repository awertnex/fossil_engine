        ...
        const str *name, const str *file, u64 line)
{
    ...
    LOGTRACEEX(0,
            file, line,
            MSG_MEM_ARENA_FREE(name, x->buf, x->buf_cap, x->entry_count, x->entry_cap));

    munmap(x->buf, x->buf_cap);
    munmap(x->freelist, x->freelist_cap);
    munmap(x->entry, x->entry_cap);
    *x = nomem_arena;
}
