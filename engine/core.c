 ...
 */
static u32 fsl_take_screenshot(const str *dir_screenshots, const str *special_text);

/* ---- section: init ------------------------------------------------------- */

u32 fsl_init(int argc, char **argv, const str *_log_dir, const str *title,
        i32 size_x, i32 size_y, fsl_render *_render, u64 flags)
{
    u32 i = 0;

    fsl_flag.active = 1;
    fsl_get_string(render->title, FSL_STR_INDEX_ENGINE_TITLE);
...
u32 fsl_process_screenshot_request(const str *dir_screenshots, const str *special_text)
{
    if (fsl_flag.request_screenshot)
    {
        ...
        return fsl_take_screenshot(dir_screenshots, special_text);
    }
