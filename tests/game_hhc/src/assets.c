...
u32 assets_init(void)
{
    ...
    if (
            ...
            fsl_mem_push_arena(&_memory_arena_internal, (void*)&texture,
            ...
            fsl_mem_push_arena(&_memory_arena_internal, (void*)&shader,
    ...
    if (
            fsl_asset_init(&shader[SHADER_DEFAULT].asset, FSL_ASSET_SHADER_PROGRAM, "Default", "default", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_DEFAULT].vertex.asset, FSL_ASSET_SHADER, "Default", "default", "default.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_DEFAULT].fragment.asset, FSL_ASSET_SHADER, "Default", "default", "default.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||

            fsl_asset_init(&shader[SHADER_SKYBOX].asset, FSL_ASSET_SHADER_PROGRAM, "Skybox", "skybox", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_SKYBOX].vertex.asset, FSL_ASSET_SHADER, "Skybox", "skybox", "skybox.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_SKYBOX].fragment.asset, FSL_ASSET_SHADER, "Skybox", "skybox", "skybox.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||

            fsl_asset_init(&shader[SHADER_GIZMO].asset, FSL_ASSET_SHADER_PROGRAM, "Gizmo", "gizmo", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_GIZMO].vertex.asset, FSL_ASSET_SHADER, "Gizmo", "gizmo", "gizmo.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_GIZMO].fragment.asset, FSL_ASSET_SHADER, "Gizmo", "gizmo", "gizmo.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||

            fsl_asset_init(&shader[SHADER_GIZMO_CHUNK].asset, FSL_ASSET_SHADER_PROGRAM, "Gizmo Chunk", "gizmo_chunk", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_GIZMO_CHUNK].vertex.asset, FSL_ASSET_SHADER, "Gizmo Chunk", "gizmo_chunk", "gizmo_chunk.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_GIZMO_CHUNK].geometry.asset, FSL_ASSET_SHADER, "Gizmo Chunk", "gizmo_chunk", "gizmo_chunk.geom", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_GIZMO_CHUNK].fragment.asset, FSL_ASSET_SHADER, "Gizmo Chunk", "gizmo_chunk", "gizmo_chunk.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||

            fsl_asset_init(&shader[SHADER_POST_PROCESSING].asset, FSL_ASSET_SHADER_PROGRAM, "Post Processing", "post_processing", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_POST_PROCESSING].vertex.asset, FSL_ASSET_SHADER, "Post Processing", "post_processing", "post_processing.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_POST_PROCESSING].fragment.asset, FSL_ASSET_SHADER, "Post Processing", "post_processing", "post_processing.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||

            fsl_asset_init(&shader[SHADER_VOXEL].asset, FSL_ASSET_SHADER_PROGRAM, "Voxel", "voxel", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_VOXEL].vertex.asset, FSL_ASSET_SHADER, "Voxel", "voxel", "voxel.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_VOXEL].geometry.asset, FSL_ASSET_SHADER, "Voxel", "voxel", "voxel.geom", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_VOXEL].fragment.asset, FSL_ASSET_SHADER, "Voxel", "voxel", "voxel.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||

            fsl_asset_init(&shader[SHADER_BOUNDING_BOX].asset, FSL_ASSET_SHADER_PROGRAM, "Bounding Box", "bounding_box", NULL, GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_BOUNDING_BOX].vertex.asset, FSL_ASSET_SHADER, "Bounding Box", "bounding_box", "bounding_box.vert", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS ||
            fsl_asset_init(&shader[SHADER_BOUNDING_BOX].fragment.asset, FSL_ASSET_SHADER, "Bounding Box", "bounding_box", "bounding_box.frag", GAME_DIR_NAME_SHADERS) != FSL_ERR_SUCCESS)
            goto cleanup;
...
{
    fsl_asset_init(&blocks[BLOCK_NONE].asset, FSL_ASSET_CUSTOM,
            "None", "none", NULL, NULL);

    fsl_asset_init(&blocks[BLOCK_BLOOD].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_GRASS].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_DIRT].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_DIRTUP].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_STONE].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_SAND].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_GLASS].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_WOOD_BIRCH_LOG].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_WOOD_BIRCH_PLANKS].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_WOOD_CHERRY_LOG].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_WOOD_CHERRY_PLANKS].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_WOOD_OAK_LOG].asset, FSL_ASSET_CUSTOM,
    ...
    fsl_asset_init(&blocks[BLOCK_WOOD_OAK_PLANKS].asset, FSL_ASSET_CUSTOM,
